...
Agent panes show the tool and reported model separately from the task title and status. The connection icons distinguish SSH from Claude Remote Control. The per-pane active count includes the main agent only while it is working, plus its live subagents; clicking the count opens the existing subagent details. Narrow sidebars shorten the count to a number and retain full descriptions in tooltips.

Claude's `SessionStart.model` and `PostModelSwitch.to_model` provide the selected session model. Codex uses a reported hook model or the newest root `turn_context` in a bounded transcript tail. An explicit unresolved model clears the previous label until a newer response supplies it. Subagent hooks cannot replace the root identity or model.

Remote Control is distinct from an SSH shell. Claude hook environments expose `CLAUDE_CODE_BRIDGE_SESSION_ID` while Remote Control is active; only the connection boolean is displayed. Matching local session records refresh that boolean through the existing two-second agent sweep, including idle connect/disconnect changes. Remote SSH sessions use hook reports and do not probe unrelated local PIDs or files. Metadata changes do not change lifecycle state, activity clocks, or completion indicators. Unknown model data is left unlabelled, with a tooltip explaining that it has not been reported.
...
`SubagentStart` / `SubagentStop` maintain a per-pane registry of running subagents (`AgentSubagentRegistryStore`, shared with Codex and Grok). Agent panes include those workers in the active count beside the model; clicking it unfolds a list grouped by model and agent type.
